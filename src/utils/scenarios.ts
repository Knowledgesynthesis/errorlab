...
export const scenarios: Scenario[] = [
  {
    ...
    description: 'Testing if a drug lowers blood pressure',
    context: 'A pharmaceutical company is testing whether a new antihypertensive medication reduces systolic blood pressure compared to placebo. Lower blood pressure is the desired outcome.',
    state: {
      alpha: 0.05,
      n: 120,
      delta: 0.6,
    ...
    metaphor: {
      h0: 'Drug does not lower blood pressure (μ_drug = μ_placebo)',
      h1: 'Drug lowers blood pressure (μ_drug < μ_placebo)',
      type1Description: 'Approving an ineffective drug - patients get false hope and potential side effects without benefit',
      type2Description: 'Rejecting an effective drug - patients miss out on a treatment that could help them',
    },
  },
  {
    id: 'bone-density',
    name: 'Osteoporosis Treatment',
    description: 'Testing if a drug increases bone density',
    context: 'Researchers are evaluating whether a new osteoporosis treatment increases bone mineral density in postmenopausal women. Higher bone density reduces fracture risk.',
    state: {
      alpha: 0.05,
      n: 150,
      delta: 0.5,
    ...
    metaphor: {
      h0: 'Treatment does not increase bone density (μ_treatment = μ_baseline)',
      h1: 'Treatment increases bone density (μ_treatment > μ_baseline)',
      type1Description: 'Approving an ineffective treatment - wasting resources and exposing patients to unnecessary risks',
      type2Description: 'Rejecting an effective treatment - denying patients a therapy that could prevent fractures',
    },
  },
  {
    id: 'drug-formulation',
    name: 'New Drug Formulation',
    description: 'Testing if formulation changes drug levels',
    context: 'A pharmaceutical company developed a new formulation of an existing medication. They need to test if it produces different blood concentration levels compared to the original formulation. Changes in either direction could affect efficacy or safety.',
    state: {
      alpha: 0.05,
      n: 200,
      delta: 0.4,
    ...
    metaphor: {
      h0: 'New formulation produces same blood levels (μ_new = μ_original)',
      h1: 'New formulation produces different blood levels (μ_new ≠ μ_original)',
      type1Description: 'Declaring bioequivalence difference when none exists - unnecessary reformulation studies and regulatory delays',
      type2Description: 'Missing a real difference in blood levels - potentially unsafe or ineffective medication reaches patients',
    },
  },
];
...
export const glossary: GlossaryTerm[] = [
  ...
  {
    term: 'z-Test vs t-Test: When to Use Each',
    symbol: 'z or t',
    definition: 'The z-test and t-test are both used to test hypotheses about means, but they differ in when they should be used. **z-Test:** Use when the population standard deviation (σ) is KNOWN and the sample is from a normal distribution (or n ≥ 30 by Central Limit Theorem). The test statistic is z = (x̄ - μ₀)/(σ/√n) and follows a standard normal distribution. **t-Test:** Use when the population standard deviation (σ) is UNKNOWN and must be estimated from the sample as s. The test statistic is t = (x̄ - μ₀)/(s/√n) and follows a t-distribution with df = n-1. The t-distribution has heavier tails than the normal distribution, accounting for the extra uncertainty from estimating σ. As n increases, the t-distribution approaches the normal distribution. **Why the difference matters:** When σ is unknown, using s introduces additional variability. The t-distribution accounts for this by having wider tails (larger critical values), making it harder to reject H₀. This is more conservative and maintains the correct Type I error rate. If you used a z-test when σ is unknown, you would underestimate uncertainty and reject H₀ too often (inflated Type I error). **In practice:** Almost always use the t-test in real research because σ is rarely known. The z-test is mainly theoretical or used in specific quality control contexts where σ is established from long-term process data.',
    example: 'Example 1 - z-test: IQ tests have a known σ = 15. Testing if a sample of 36 students has higher IQ than μ₀ = 100: use z-test because σ is known. Example 2 - t-test: Testing if a new blood pressure medication lowers BP. You don\'t know the population σ, only the sample SD s = 12 from n = 50 patients: use t-test because σ is unknown. With df = 49, the t critical value (≈2.01) is slightly larger than the z critical value (1.96) for α = 0.05 two-sided.',
  },
];
